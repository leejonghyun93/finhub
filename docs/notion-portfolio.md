...
| **개발 방식** | Claude Code(AI) 활용 바이브코딩 + 직접 학습/검증 |
...
**4. AI를 활용한 바이브코딩으로 개발 생산성 극대화**
- Claude Code를 페어 프로그래머로 활용해 MSA 전체 구조 설계 및 구현
- 단순히 코드를 받아쓰는 것이 아니라 각 기술 선택의 이유를 이해하며 진행
- AI가 생성한 코드도 반드시 직접 실행/검증 (JwtTokenProvider 생성자 불일치 이슈 등)
- AI 도구 활용 능력 자체가 현대 개발자의 핵심 역량임을 체감
...
**2. 통합 테스트 커버리지 확대 필요**
현재 finhub-user와 finhub-banking에만 H2 인메모리 DB 기반 통합 테스트가 구현되어 있습니다. finhub-investment, finhub-payment, finhub-insurance 등 나머지 서비스의 통합 테스트가 미구현 상태입니다.
